...
以 DeepSeek V4 Flash 为例：按 2026-07-23 的未命中缓存标准 API 价，把“一万字”粗略按“约一万输入 Token，加约一万输出 Token”估算，合计约 **0.03 元人民币，也就是约 3 分钱**。不同语言、输出长度、缓存命中与实时价格都会变化；整体消耗通常远低于一些付费会员的使用门槛。实际费用以用户所选 Provider 的实时价格和账单为准，FloatRead 不加价、不经手费用。
...
用户明确启动后，FloatRead 只扫描当前可见、且属于用户所选源语言的文本。语言判断在本地完成；它使用语义化 HTML 角色，不依赖 X 私有的 `data-testid`。精细、智能、快速档分别最多处理 6/6,000、8/8,000、12/12,000 个片段/字符，并在滚动时处理新出现内容，不预读无限时间线。页面进入后台后不提交新批次。
