...
FloatRead 是一个开源、无开发者服务器、用户自带 API Key 的 Chromium 浏览器悬浮阅读助手。选中英文文本并主动触发后，它会在独立面板中提供简洁的中文阅读帮助，同时不改变原网页内容和布局。

> 当前版本：`0.1.0`。源码中的发布者链接是集中管理的临时默认值；公开发布前应修改 `src/config/branding.ts`。
...
- Popup、当前网站暂停、全局暂停、右键菜单、快捷键和首次引导。
- 中英文界面、键盘操作、深浅色兼容和减少动画支持。
- OpenAI、OpenAI Compatible、DeepSeek、Anthropic Claude、Google Gemini 和 Ollama。
- Native、Lens、Glass Orb、Pixel Bot、Ink、Terminal 六套内置皮肤及安全社区皮肤导入。

FloatRead 没有开发者后端、账号、支付、会员、广告、埋点、遥测或使用统计。Provider 请求由扩展 Background Service Worker 直接发往用户配置的服务地址。

## 为什么不改变 X 页面

FloatRead 不向推文插入按钮或结果，不改变推文高度，不扫描时间线，不依赖 X 内部选择器，也不注入全局 CSS。页面中只会出现一个固定定位的 `floatread-root`，FloatRead 全部界面都在它的 closed Shadow DOM 内。暂停或隐藏后，宿主节点会被完整移除。

只有用户对当前选中文字主动操作后才会产生 AI 请求；未选中的网页内容不会被收集。
...
1. 获取 `FloatRead-v0.1.0.zip`，并用相邻 `.sha256` 文件校验摘要。
2. 将 ZIP 解压到固定的本地目录。
...
进入“设置 → Provider”，选择适配器，核对 Base URL 和模型名，选择 Key 保存模式，再授予精确 Host 权限并点击“测试连接”。自定义接口优先选择 OpenAI Compatible。
...
选中文字和模型输出可能按用户选择发送给对应 Provider，并受该 Provider 条款约束。FloatRead 没有接收数据的开发者服务器，不收集统计。模型输出按文本渲染；外部 JSON 使用 Schema 校验；皮肤包不能包含可执行内容；生产构建会检查秘密和远程代码。

请阅读 [PRIVACY.md](PRIVACY.md)、[SECURITY.md](SECURITY.md) 和 [威胁模型](docs/THREAT_MODEL.md)。安全漏洞不要通过公开 Issue 披露。

## 皮肤制作与导入

社区包是本地 `.floatread-skin` ZIP，只允许严格 JSON 与 PNG/WebP。JavaScript、HTML、SVG、CSS、字体、远程 URL 和可执行表达式都会被拒绝。制作方法与容量限制见 [皮肤文档](docs/SKINS.md)。
