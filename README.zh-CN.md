...
> 当前版本：`0.2.3`。0.2.2 的 React 文本节点即时重应用可能让动态页面过载，已被废止，不应继续在 X 使用。源码中的发布者链接是集中管理的临时默认值；公开发布前应修改 `src/config/branding.ts`。
...
用户明确启动后，FloatRead 只扫描可见和接近视口的英文文本。它使用语义化 HTML 角色，不依赖 X 私有的 `data-testid`；普通批次最多处理 6 个片段/6,000 字符，并在滚动时处理新出现内容，不预读无限时间线。
...
1. 获取 `FloatRead-v0.2.3.zip`，并用相邻 `.sha256` 文件校验摘要。
2. 将 ZIP 解压到固定的本地目录。
