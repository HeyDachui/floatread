...
> 当前版本：`0.4.0`。页面翻译默认只处理英语；用户可选择快速、智能或精细档，添加其他源语言并选择统一目标语言。源码中的发布者链接是集中管理的临时默认值；公开发布前应修改 `src/config/branding.ts`。
...
1. 获取 `FloatRead-v0.4.0.zip`，并用相邻 `.sha256` 文件校验摘要。
2. 将 ZIP 解压到固定的本地目录。
