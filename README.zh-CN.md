...
> 当前版本：`0.2.2`。源码中的发布者链接是集中管理的临时默认值；公开发布前应修改 `src/config/branding.ts`。
...
用户明确为当前 Origin 开启后，FloatRead 只扫描可见和接近视口的英文文本。它使用语义化 HTML 角色，不依赖 X 私有的 `data-testid`；每批最多处理 12 个受限片段，并在滚动时处理新出现内容，不预读无限时间线。

译文会替换对应可见文本节点，因此文字换行可能变化。受限观察器只在页面翻译开启时检测动态推文和菜单，不修改 React 事件或 X 业务状态。停止会取消当前批次并保留已有译文；清除会恢复仍然存在的原文节点并关闭该网站的持久翻译授权。
...
1. 获取 `FloatRead-v0.2.2.zip`，并用相邻 `.sha256` 文件校验摘要。
2. 将 ZIP 解压到固定的本地目录。
